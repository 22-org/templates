"""
Test Case: Learning Resource Discovery
Discover relevant learning resources across platforms
"""
...
def test_learning_resource_discovery(token_data: TokenData, project_id: str):
    ...
    template = (
        "Discover {resource_types} resources for {topic} at {difficulty} "
        "level on {platforms} platforms with {free_preference} free preference"
    )

    payload = {"sequence_data": sequence_data, "template": template}

    try:
        print("=== Testing Learning Resource Discovery ===")
        ...
        if response.status_code == 200:
            ...
            print(f"Learning Resource Discovery: {result}")
            return result
        else:
...
if __name__ == "__main__":
    ...
    project_id = generate_project(
        name="Learning Resource Discovery Test",
        description="Test project for learning resource discovery",
    )

    # Make recommendations
    test_learning_resource_discovery(token_data, project_id)

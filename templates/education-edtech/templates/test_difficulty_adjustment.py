...
def test_difficulty_adjustment(token_data: TokenData, project_id: str):
    ...
    sequence_data = {
        ...
        "recent_scores": [65, 70, 68],
        "learning_speed": "slow",
        "confidence_level": "low",
    }

    template = (
        "Adjust difficulty for {subject} based on {current_performance} "
        "performance with scores {recent_scores} and {learning_speed} "
        "learning speed"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Difficulty Adjustments: {result}")
            return result
        else:
...
if __name__ == "__main__":
    ...
    project_id = generate_project(
        name="Difficulty Adjustment Test",
        description="Test project for difficulty adjustment",
    )

    # Make recommendations
    test_difficulty_adjustment(token_data, project_id)
